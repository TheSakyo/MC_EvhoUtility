...
public class ImageHelper {
	...
	public static BufferedImage getImage(String path, Player player) throws IOException {
	  ...
	  //Sinon, si l'argument "path" est un emplacement d'un fichier, on essaie de le trouver dans le dossier '/utils/maps/images/'
	  } else {
		  ...
		  if(ImageFile.exists()) { return ImageIO.read(ImageFile); }
			  ...
			  if(player == null) throw new IOException(ChatFormatting.RED + "L'Image spécifiée est introuvable dans /utils/maps/images/ ou le lien est incorrect !");
			  else {
			
				 player.sendMessage(UtilityMain.getInstance().prefix + ChatFormatting.RED + "L'Image spécifiée est introuvable dans /utils/maps/images/ ou le lien est incorrect !");
