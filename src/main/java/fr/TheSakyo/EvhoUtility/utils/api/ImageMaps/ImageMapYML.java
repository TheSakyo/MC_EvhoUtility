...
public class ImageMapYML {
	...
	public void write(ImageMap imagemap, ItemStack item) {
		...
		config.set("url", imagemap.getPath()); //Enregistre le lien ou le fichier de "l'imagemap" qui a été définit à la création
	...
	public ImageMap read() {
		...
		final String URLStr = config.getString("url"); //Récupère le lien ou le fichier de "l'imagemap" qui a été définit à la création enregistrée
