...
public class NMSEntityData {
  ...
  /**
   * Charge les méthodes du NMS pour travailler avec les métadonnées des entitées
   *
   */
  public static void load() throws ClassNotFoundException, NoSuchMethodException {
    ...
    // ⬇️ Initialise les méthodes en fonction de la version actuelle du Serveur ⬇️ //
    if(serverVersion.isOlderThanOrEqual(ServerVersion.VERSION_1_17_1)) set = SynchedEntityData.class.getMethod("set", EntityDataAccessor.class, Object.class);
    else {
  ...
  /**
   * Ajoute des données de métadonnés pour une Entité en question
   *
  ...
  /**
   * Récupère la liste des données de métadonnés pour l'Entité en question
   *
   * @param synchedEntityData Métadonnées de l'Entité
   */
