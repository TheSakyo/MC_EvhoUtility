...
public class NMSUtils {
   ...
   *
   * @param nmsClassString L'Emplacment préfix en question de la 'class' a récupéré
   * @param nmsClassString La 'class' en question a récupéré
   *
   */
  ...
  /**
   * Récupère une variable précis dans une 'class' précise du 'NMS'
   *
   * @param instance La 'class' NMS dans laquelle vérifiée la variable
   * @param name Le nom de la variable a récupéré
   *
   */
  ...
  public static Object getValue(Object instance, String name, boolean printError) {
    ...
    try {
      ...
      field.setAccessible(true); // On autorise l'accesibilité à cette variable
      result = field.get(instance); // On récupère la valeur de cette variable et la définit dans la variable de résultat initialiser plus haut
      field.setAccessible(false); // On enlève l'accesibilité à cette variable

    } catch(Exception e) { if(printError) e.printStackTrace(); /* Une erreur s'affiche, si cela a été demandé */ }
    // ⬆️ On essaie de récupérer une variable en question à partir d'une 'class' du NMS en question, si une erreur survient, on affiche l'erreur, si cela a été demandé ⬆️ //
