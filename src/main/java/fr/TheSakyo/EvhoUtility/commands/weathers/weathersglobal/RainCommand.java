		...
		if(sender instanceof Player p) {
			...
			if(p.hasPermission("evhoutility.weather")) {
				...
				if (args.length == 0) {
					...
					p.sendMessage(main.prefix + ChatColor.GRAY.toString() + ChatColor.ITALIC.toString() + "Vous avez définit la météo étant un temps pluvieux dans votre monde");
					
				} else if (args.length != 0) {
					
					p.sendMessage(main.prefix + ChatColor.RED + "Essayez /rain sans arguments");
				}
