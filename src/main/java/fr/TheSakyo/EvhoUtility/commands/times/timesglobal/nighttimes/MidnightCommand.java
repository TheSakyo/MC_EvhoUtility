		...
		if(sender instanceof Player p) {
			...
			if(p.hasPermission("evhoutility.time")) {
				...
				if(args.length == 0) {
					...
					p.sendMessage(main.prefix + ChatColor.GRAY.toString() + ChatColor.ITALIC.toString() + "Vous avez définit le temps étant minuit dans votre monde");
					
				} else if(args.length != 0) {
					
					p.sendMessage(main.prefix + ChatColor.RED + "Essayez /midnight sans arguments");
				}
