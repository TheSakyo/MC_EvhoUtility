		...
		if(sender instanceof Player p) {
			...
			if(p.hasPermission("evhoutility.time")) {
				...
				if(args.length == 0) {
					...
					p.sendMessage(main.prefix + ChatFormatting.GRAY.toString() + ChatFormatting.ITALIC.toString() + "Vous avez définit le temps étant la nuit dans votre monde");
					
				} else p.sendMessage(main.prefix + ChatFormatting.RED + "Essayez /night sans arguments");
