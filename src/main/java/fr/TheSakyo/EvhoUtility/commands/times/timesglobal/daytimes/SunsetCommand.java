		...
		if(sender instanceof Player p) {
			...
			if(p.hasPermission("evhoutility.time")) {
				...
				if(args.length == 0) {
					...
					p.sendMessage(main.prefix + ChatFormatting.GRAY.toString() + ChatFormatting.ITALIC.toString() + "Vous avez définit le temps étant le coucher de soleil dans votre monde");
					
				} else p.sendMessage(main.prefix + ChatFormatting.RED + "Essayez /sunset sans arguments");
